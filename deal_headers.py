...
class DHeader(Resource):

    def get(self):
        args = parser.parse_args()
        print(args)
        res = html2header(args['header'])
...
class Login(Resource):
    ...
    def post(self):
        data = {'msg': 'error'}
        if request.form['username'] == 'jola' and request.form['password'] == '123456':
            data = {
                'msg': 'success',
                'code': 200,
                'user': 'jola'
...
api.add_resource(DHeader, '/dealheader')
api.add_resource(Login, '/login')


def html2header(text=''):
